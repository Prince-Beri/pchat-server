...
import { deleteFilesFromCloudinary, emitEvent, uploadFilesToCloudinary } from "../utils/features.utils.js";
import { ALERT, NEW_MESSAGE, NEW_MESSAGE_ALERT, REFETCH_CHATS } from '../constants/event.constants.js'
...
const addMembers = TryCatch(async (req, res, next) => {
   ...
   // updating user chat list after adding them into group.
...
const leaveGroup = TryCatch(async (req, res, next) => {
   ...
   if(chat.creator.toString() === req.userId.toString()){
        const rendomElement = Math.floor(Math.random() * remaningMembers.length);
        const newCreator = remaningMembers[rendomElement];
...
const deleteChat = TryCatch(async (req, res, next) => {
    ...
    if(!chat.groupChat && !chat.members.includes(req.userId.toString())){
        return next(new ErrorHandler('You are not allowed to delete the chat.'));
    }

    // Here we have to delete all the messages as well attachments or files form cloudinary.
